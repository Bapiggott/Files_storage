...
class MiniSpecProgram:
    ...
    def translate_to_python(self, minispec_stmt: str) -> str:
        ...
        if minispec_stmt[0].isdigit():
            print("minispec_stmt[0].isdigit():")
            #print(minispec_stmt[1])
            #if minispec_stmt.endswith('{'):
            print("minispec_stmt.endswith('{'):")
            loop_count = int(minispec_stmt.split('{')[0].strip())
            return self.get_indent() + f"for _ in range({loop_count}):"
        await_function = ['move_forward','move_backward', 'move_left','move_right','move_up','move_down', 'turn_cw', 'turn_ccw', 'move_in_circle', 'delay','orienting', 'approach', 'scan', 'scan_abstract', 'goto']
        ...
        for abbr, func_name in self.abbreviations.items():
            if minispec_stmt.startswith(abbr + '('):
                ...
                if func_name in await_function:
        ...
        if minispec_stmt.split('(', 1)[0] in self.functions:
            ...
            if func_name in self.functions:
                ...
                if func_name in await_function:
                    return self.get_indent() + "await" + func_name + '(' + minispec_stmt[len(func_name) + 1:-1] + ')'
                else:
...
if __name__ == '__main__':
    code = "4{_1=ox($1);?_1>0.6{tc(15)};?_1<0.4{tu(15)};_2=ox($1);?_2<0.6&_2>0.4{->True}}->False;"#"5 { _1 = p('Any animal target here?');?_1 != False { l(_1);-> True; }tc(30);} -> False;"
