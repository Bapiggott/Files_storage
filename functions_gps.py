...
async def reached_target_altitude(target_altitude, starting_altitude, tolerance=0.009, stability_count=15):
...
async def move_up(distance: float):
    if await check_connection_with_retry():
        ...
        print(f"Moving up to {new_altitude} meters")
        ...
        # Use a timeout to wait until the drone reaches the target altitude
        await timeout_check(2000, lambda: asyncio.ensure_future(reached_target_altitude(changing_elevation, starting_altitude)))
...
async def check_connection_with_retry(retry_limit=7):
    global drone
    retries = 0
