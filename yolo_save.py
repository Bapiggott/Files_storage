...
def run_yolo():
    try:
        ...
        results.render()  # This updates results.imgs with the rendered images

        # Get the image with detections (since we're processing one image, it's at index 0)
        img_with_boxes = results.imgs[0]

        # Convert the image back to bytes
        _, img_encoded = cv2.imencode('.jpg', img_with_boxes)
        img_bytes_with_boxes = img_encoded.tobytes()

        # Save the image with detections to the uploads folder for the latest image endpoint
        image_path = os.path.join(app.config['UPLOAD_FOLDER'], file.filename)
        with open(image_path, 'wb') as f:
            f.write(img_bytes_with_boxes)
