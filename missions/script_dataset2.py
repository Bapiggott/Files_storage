...
def process_json(json_path):
    with open(json_path, 'r') as file:
        ...
        while True:
            chunk = file.read(4096)  # Read a chunk of the file
            if not chunk:
                break  # End of file
            ...
            try:
                ...
                obj, idx = decoder.raw_decode(partial_json)
                partial_json = partial_json[idx:].lstrip()  # Remove processed portion

                # Process the decoded JSON object
                if "frame.protocols\":\"sll:ethertype:ip:" in json.dumps(
                        obj) or "frame.protocols\": \"sll:ethertype:ip:" in json.dumps(obj):
                    remaining_content = json.dumps(obj)[idx:]
                ...
                pass  # Continue accumulating if the content is incomplete or not yet valid JSON
...
def remove_incomplete_package(file_path):
